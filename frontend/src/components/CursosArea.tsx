    ...
    <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
        ...
        <div className="mt-4 space-y-2">
          {curso.materias?.map(m => (
            <Button
              ...
              onClick={() => { setSel(m); setSc(null); }}
            >
              {m.nome} ({m.conteudos?.length})
            </Button>
          ))}
        </div>
        <Button onClick={onVoltar} className="mt-6 w-full">← Voltar</Button>
      </aside>
        ...
        {!sel ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sel.conteudos?.map(c => (
                <Card key={c.id} onClick={() => setSc(c)} className="cursor-pointer">
                  <CardHeader>
            ...
            {sc && <div className="mt-6"><ContentViewer conteudo={sc} /></div>}
          </>
        )}
      </main>
      ...
      {modalMat && (
        <Modal title="Nova Matéria" onClose={() => setModalMat(false)}>
          ...
          />
          <Button onClick={adicionarMateria} className="mt-4 w-full bg-green-600 text-white">
            Criar
          </Button>
        </Modal>
      )}
      ...
      {modalCont && (
        <Modal title="Novo Conteúdo" onClose={() => setModalCont(false)}>
          <Input placeholder="Título" value={tt} onChange={e => setTt(e.target.value)} />
          <Select value={ct} onValueChange={v => setCt(v as any)}>
            <SelectTrigger><SelectValue obrigatório />
            <SelectContent>
          ...
          <Input placeholder="URL ou arquivo" value={arq} onChange={e => setArq(e.target.value)} />
          <Button onClick={adicionarConteudo} className="mt-4 w-full bg-green-600 text-white">
            Adicionar
          </Button>
        </Modal>
      )}
